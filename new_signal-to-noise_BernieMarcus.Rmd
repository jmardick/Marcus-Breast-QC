...
sample_dir <- gsub("\\\\", "/", r"(C:\Users\Jacob\Documents\Breast-BernieMarcus\Test)")
...
for (mz in unique_mz) {
  ...
  xall_subset <- xall_subset |> 
    mutate(class = factor(class, levels = c("Cancer", "Normal"))) |>
    arrange(class, Sample)
  
  ## Convert Sample to a factor for proper ordering  
  ...
  divider_position <- sum(xall_subset$class == "Cancer") + 0.5
  
  ## Average intensity for plot line
  avg_intensity <- mean(xall_subset$Intensity, na.rm = TRUE)
  y_max <- max(xall_subset$Intensity, na.rm = TRUE) * 1.1  

  ## Generate plot
  plot <- ggplot(xall_subset, aes(x = Sample, y = Intensity)) +
    ## Background shading
    annotate("rect", xmin = 0, xmax = divider_position - 0.5, ymin = -Inf, ymax = Inf, fill = "lightcoral", alpha = 0.3) +
    annotate("rect", xmin = divider_position, xmax = Inf, ymin = -Inf, ymax = Inf, fill = "lightblue", alpha = 0.3) +
    ## Horizontal line for average intensity
    geom_hline(yintercept = avg_intensity, linetype = "solid", color = "#009E73", linewidth = 1) +
    ## Label average intensity line
    annotate("text", x = nrow(xall_subset) * 0.82, y = y_max * 0.95, label = paste0("Average Median Log Intensity: ", round(avg_intensity, 2)), 
             color = "#009E73", hjust = 0, size = 4) +
    ...
    ## Labels for Normal and Cancer groups
    annotate("text", x = divider_position / 2, y = y_max, label = "Cancer", color = "red", size = 5, fontface = "bold") +
    annotate("text", x = divider_position + (nrow(xall_subset) - divider_position) / 2, y = y_max, label = "Normal", 
             color = "blue", size = 5, fontface = "bold") +
    ## Improve theme
    theme_minimal() +
    labs(title = paste("Intensities for m/z", mz), x = "Sample", y = "Median Log Normalized Intensity") +
...
for (mz in unique_mz) {
  ...
  ## Sort samples by class (i.e., tumor or normal)
  xall_snr_subset <- xall_snr_subset |> 
    mutate(class = factor(class, levels = c("Cancer", "Normal"))) |> 
    arrange(class, Sample)
  ...
  divider_position <- sum(xall_snr_subset$class == "Cancer") + 0.5
  
  ## Calculate average SNR for this m/z
  avg_snr <- mean(xall_snr_subset$SNR, na.rm = TRUE)
  ...
  plot <- ggplot(xall_snr_subset, aes(x = Sample, y = SNR)) +
    ...
    annotate("rect", xmin = 0, xmax = divider_position - 0.5, ymin = -Inf, ymax = Inf, fill = "lightcoral", alpha = 0.3) + 
    annotate("rect", xmin = divider_position, xmax = Inf, ymin = -Inf, ymax = Inf, fill = "lightblue", alpha = 0.3) +
    ## Horizontal line for average SNR
    geom_hline(yintercept = avg_snr, linetype = "solid", color = "#009E73", linewidth = 1) +
    ## Label average SNR line
    annotate("text", x = nrow(xall_snr_subset) * 0.85, y = y_max * 0.95, 
             label = paste0("Average SNR: ", round(avg_snr, 2)), 
             color = "#009E73", hjust = 0, size = 4) +
    ## Add class labels
    geom_vline(xintercept = divider_position, linetype = "dashed", color = "black") +
    geom_point(color = "black", size = 2, alpha = 0.7) +
    geom_line(aes(group = 1), color = "gray") +
    annotate("text", x = divider_position / 2, y = y_max, label = "Cancer", color = "red", size = 5, fontface = "bold") +
    annotate("text", x = divider_position + (nrow(xall_snr_subset) - divider_position) / 2, 
             y = y_max, label = "Normal", color = "blue", size = 5, fontface = "bold") +
    theme_minimal() +
    labs(title = paste("Signal-to-Noise Ratios for m/z", mz),
         x = "Sample",
         y = "Signal-to-Noise Ratio") +
    scale_x_discrete() +  ## Ensure Sample is treated as discrete
    theme(plot.margin = margin(10, 25, 10, 25, "pt"),
          axis.text.x = element_text(angle = 45, hjust = 1, size = 6),  ## Larger x-axis text
          plot.title = element_text(hjust = 0.5, face = "bold", size = 16),
          axis.title.x = element_text(size = 14, face = "bold"),
          axis.title.y = element_text(size = 14, face = "bold"),
          axis.text.y = element_text(size = 12))
